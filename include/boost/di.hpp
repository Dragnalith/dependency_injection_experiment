...
struct is_supported
    : aux::is_same<aux::bool_list<aux::always<TDeps>::value...>,
          aux::bool_list<(aux::is_constructible<TDeps, TDeps&&>::value && (aux::is_a<core::injector_base, TDeps>::value || aux::is_a<core::dependency_base, TDeps>::value || aux::is_empty_expr<TDeps>::value))...>> {
};
template <class...>
...
struct array<T(), Ts...> : T {
    ...
    template <__BOOST_DI_REQUIRES(
                  aux::is_constructible<T, std::move_iterator<value_type*>, std::move_iterator<value_type*>>::value)
        = 0>
...
public:
    ...
    struct scope<TExpected, TGiven, __BOOST_DI_REQUIRES(aux::is_callable<TGiven>::value)> {
        ...
        template <class T, class, class TProvider,
            __BOOST_DI_REQUIRES(!detail::is_expr<TGiven, TProvider>::value && aux::is_invocable<TGiven>::value && !aux::is_callable<TExpected>::value) = 0>
...
struct try_provider<aux::pair<T, aux::pair<TInitialization, TList<TCtor...>>>, TInjector, TProvider> {
    ...
    template <class>
    struct is_creatable {
        static constexpr auto value = TProvider::template is_creatable<TInitialization, T,
...
protected:
    ...
    struct is_creatable {
        ...
        static constexpr auto value = aux::is_convertible<decltype(dependency__<dependency_t>::template try_create<T, TName>(
...
public:
  ...
  create
    ...
    template <template <class...> class T,
        __BOOST_DI_REQUIRES(
            is_creatable<binder::resolve_template_t<injector, aux::identity<T<>>>, no_name, aux::true_type>::value)
        = 0>
...
#else
#define __BOOST_DI_IBP_SPLIT(i, ...) __BOOST_DI_PRIMITIVE_CAT(__BOOST_DI_IBP_SPLIT_, i) \
(__VA_ARGS__)
#define __BOOST_DI_IBP_SPLIT_0(a, ...) a
#define __BOOST_DI_IBP_SPLIT_1(a, ...) __VA_ARGS__
